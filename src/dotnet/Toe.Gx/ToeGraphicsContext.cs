...
namespace Toe.Gx
{
	public class ToeGraphicsContext : IDisposable
	{
		...
		public void Render(IVertexSource mesh)
		{
			var p = ApplyMaterialShader(mesh);
			switch (mesh.VertexSourceType)
			{
		...
		private ShaderTechniqueArgumentIndices ApplyMaterialShader(IVertexSource mesh)
		{
			if (material == null)
				return null;
			...
			EvalShaderTechniqueArguments();

			if (p.inPMVMat >= 0) GL.UniformMatrix4(p.inPMVMat, false, ref args.inPMVMat);
			if (p.inMVMat >= 0) GL.UniformMatrix4(p.inMVMat, false, ref args.inMVMat);
			if (p.inMVRotMat >= 0) GL.UniformMatrix4(p.inMVRotMat, false, ref args.inMVRotMat);
			if (p.inModelRotMat >= 0) GL.UniformMatrix4(p.inModelRotMat, false, ref args.inModelRotMat);
			if (p.inModelPos >= 0) GL.Uniform3(p.inModelPos, ref args.inModelPos);
			if (p.inCamPos >= 0) GL.Uniform3(p.inCamPos, ref args.inCamPos);
			if (p.inAlphaTestValue >= 0) GL.Uniform1(p.inAlphaTestValue, args.inAlphaTestValue);
			if (p.inMaterialAmbient >= 0) GL.Uniform4(p.inMaterialAmbient, args.inMaterialAmbient);
			if (p.inMaterialDiffuse >= 0) GL.Uniform4(p.inMaterialDiffuse, args.inMaterialDiffuse);
			if (p.inSampler0 >= 0) GL.Uniform1(p.inSampler0, 0);
			if (p.inSampler1 >= 0) GL.Uniform1(p.inSampler1, 0);
			if (p.inSampler2 >= 0) GL.Uniform1(p.inSampler2, 0);
			if (p.inSampler3 >= 0) GL.Uniform1(p.inSampler3, 0);
			if (p.inEmissive >= 0) GL.Uniform4(p.inEmissive, args.inEmissive);
			if (p.inAmbient >= 0) GL.Uniform4(p.inAmbient, args.inAmbient);
			if (p.inDiffuse >= 0) GL.Uniform4(p.inDiffuse, args.inDiffuse);
			if (p.inDiffuseDir >= 0) GL.Uniform3(p.inDiffuseDir, args.inDiffuseDir);
			if (p.inFogNear >= 0) GL.Uniform1(p.inFogNear, args.inFogNear);
			if (p.inFogRange >= 0) GL.Uniform1(p.inFogRange, args.inFogRange);
			if (p.inFogColour >= 0) GL.Uniform4(p.inFogColour, args.inFogColour);
			if (p.inUVOffset >= 0) GL.Uniform2(p.inUVOffset, args.inUVOffset);
			if (p.inUVScale >= 0) GL.Uniform2(p.inUVScale, args.inUVScale);
			if (p.inUV1Scale >= 0) GL.Uniform2(p.inUV1Scale, args.inUV1Scale);
			if (p.inUV2Scale >= 0) GL.Uniform2(p.inUV2Scale, args.inUV2Scale);
			if (p.inUV3Scale >= 0) GL.Uniform2(p.inUV3Scale, args.inUV3Scale);
			if (p.inTextureSize >= 0) GL.Uniform2(p.inTextureSize, args.inTextureSize);
			if (p.inTextureSize1 >= 0) GL.Uniform2(p.inTextureSize1, args.inTextureSize1);
			if (p.inTextureSize2 >= 0) GL.Uniform2(p.inTextureSize2, args.inTextureSize2);
			if (p.inTextureSize3 >= 0) GL.Uniform2(p.inTextureSize3, args.inTextureSize3);
			if (p.inOOTextureSize >= 0) GL.Uniform2(p.inOOTextureSize, args.inOOTextureSize);
			if (p.inOOTextureSize1 >= 0) GL.Uniform2(p.inOOTextureSize1, args.inOOTextureSize1);
			if (p.inOOTextureSize2 >= 0) GL.Uniform2(p.inOOTextureSize2, args.inOOTextureSize2);
			if (p.inOOTextureSize3 >= 0) GL.Uniform2(p.inOOTextureSize3, args.inOOTextureSize3);
			if (p.inDisplaySize >= 0) GL.Uniform2(p.inDisplaySize, args.inDisplaySize);
			if (p.inDeviceSize >= 0) GL.Uniform2(p.inDeviceSize, args.inDeviceSize);
			if (p.inDisplayRotScaleMat >= 0) GL.UniformMatrix4(p.inDisplayRotScaleMat, false, ref args.inDisplayRotScaleMat);
		...
		private void EvalShaderTechniqueArguments()
		{
			...
			var invMVMat = args.inMVMat;
			invMVMat.Invert();
			args.inMVRotMat = args.inMVMat;
			args.inModelRotMat = model;
			args.inModelPos = Vector3.Transform(Vector3.Zero, model);
			args.inCamPos = Vector3.Transform(Vector3.Zero, view);
			args.inDiffuseDir = Vector3.Transform(this.light.Position, invMVMat);
			...
			args.inDiffuse = light.Enabled ? ColorToVector4(light.Diffuse) : new Vector4(0, 0, 0, 1.0f);
